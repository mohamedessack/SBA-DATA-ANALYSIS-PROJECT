...
Select count(LoanNumber) as Loans_Approved, sum(cast(InitialApprovalAmount AS float) ) Total_Net_Dollars, AVG (cast(InitialApprovalAmount AS float)) Average_Loan_Size, 
...
Select top 15 OriginatingLender, count(LoanNumber) as Loans_Approved, sum(cast(InitialApprovalAmount AS float)) Net_Dollars, AVG(cast(InitialApprovalAmount AS float)) Average_Loan_Size
from [dbo].[sba_public_data]
where year(DateApproved) = 2021
...
with cte as (
	...
	select ncd.Sector, count(LoanNumber) as Loans_Approved, sum(cast(CurrentApprovalAmount AS float) ) Net_Dollars
...
select BorrowerState as state, count(LoanNumber) as Loan_Count, sum(cast(CurrentApprovalAmount AS float)) Net_Dollars
from [dbo].[sba_public_data] main
...
select race, count(LoanNumber) as Loan_Count, sum(cast(CurrentApprovalAmount AS float)) Net_Dollars
from [dbo].[sba_public_data]
group by race
order by 3

select gender, count(LoanNumber) as Loan_Count, sum(cast(CurrentApprovalAmount AS float)) Net_Dollars
from [dbo].[sba_public_data]
group by gender
order by 3

select Ethnicity, count(LoanNumber) as Loan_Count, sum(cast(CurrentApprovalAmount AS float)) Net_Dollars
from [dbo].[sba_public_data]
group by Ethnicity
order by 3

select Veteran, count(LoanNumber) as Loan_Count, sum(cast(CurrentApprovalAmount AS float)) Net_Dollars
from [dbo].[sba_public_data]
group by Veteran
order by 3
...
select count(LoanNumber) as Count_of_Payments, sum(cast(ForgivenessAmount AS float)) Forgiveness_amount_paid
from sba_public_data
where year(DateApproved) = 2020 and cast(ForgivenessAmount AS float) <> 0

---Summary of 2021 PPP Approved Lending
Select count(LoanNumber) as Loans_Approved, sum(cast(InitialApprovalAmount AS float)) Total_Net_Dollars, sum(cast(ForgivenessAmount AS float)) Forgiveness_amount_paid,
(select count(distinct (OriginatingLender))from [dbo].[sba_public_data] where year(DateApproved) = 2021)Total_Originating_Lender_Count
from [dbo].[sba_public_data]
where year(DateApproved) = 2020 
order by 3 desc
...
select Year(DateApproved) Year_Approved, Month(DateApproved)Month_Approved, ProcessingMethod, sum(cast(CurrentApprovalAmount AS float)) Net_Dollars
from sba_public_data
